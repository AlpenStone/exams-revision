...
const subjects: Record<string, SubjectConfig> = {
  ...
  ins: {
    ...
    title: 'INS Notes',
    items: [
      { id: 'ww1-causes', label: 'Causes of WW1', icon: <Globe className="w-5 h-5" /> }
    ]
  }
};
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-2 relative">
        {activeSubject.items.map((item) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center justify-center shrink-0 w-6 h-6 transition-colors ${
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-2 rounded-lg transition-colors w-full text-left ${
                      isSubjectActive ? 'bg-indigo-50 text-indigo-700 font-medium' : 'hover:bg-slate-50 text-slate-600'
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold shrink-0 text-xs ${
                      isSubjectActive ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'
                    }`}>
